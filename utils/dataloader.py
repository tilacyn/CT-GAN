...
class DataLoader():
    def __init__(self, dataset_path, normdata_path, img_res=None):
        ...
        self.data_len = len(os.listdir(dataset_path))
        self.indices = np.random.permutation(self.data_len)
        self.dataset_path = dataset_path

        # format for nerual net
        # todo self.data_train = self.data_train.reshape((len(self.data_train), self.img_res[0], self.img_res[1], self.img_res[2], 1))
        # shuffle
        # np.random.shuffle(self.data_train)

    def load_image(self, i):
        image = np.load(os.path.join(self.dataset_path, '{}.npy'.format(str(i))))
        image = image.reshape((self.img_res[0], self.img_res[1], self.img_res[2], 1))
        return image

    def load_data(self, batch_size=1, is_testing=False):
        idx = np.random.permutation(self.data_len)
        idx_to_load = idx[:batch_size]
        batch_images = [self.load_image(i) for i in idx_to_load]
        ...
        for i, img in enumerate(batch_images):
            imgs_A.append(img)
            img_out = np.copy(img)
            img_out[self.m_zlims[0]:self.m_zlims[1], self.m_xlims[0]:self.m_xlims[1],
            self.m_ylims[0]:self.m_ylims[1]] = 0
    ...
    def load_batch(self, batch_size=1, is_testing=False):
        ...
        for i in range(self.n_batches - 1):
            ...
            batch = [self.load_image(i) for i in idx]
            imgs_A = []
            imgs_B = []
